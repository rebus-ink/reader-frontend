  ...
  try {
    ...
    const setup = (node, info) => {
      ...
      if (oa.length) {
        ...
        const changes = []
        for (let i = 0, length = oa.length; i < length; i++) { changes.push({ attributeName: oa[i], oldValue: null, target: node }) }
      ...
      define: {
        value (name, Class, options) {
          ...
          if (options && EXTENDS in options) {
            ...
            for (let i = 0, length = changes.length; i < length; i++) { setupIfNeeded(changes[i]) }
