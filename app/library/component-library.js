...
export const Library = component(({request}, {dispatch}) => {
  ...
  <option value="${`${pathname}?order=alpha`}" selected=${query.get('order') === 'alpha' && !query.get('desc')}>A-Z</option>
  <option value="${`${pathname}?order=alpha&desc=true`}" selected=${query.get('order') === 'alpha' && query.get('desc') === 'true'}>Z-A</option>
  </select></p>
    </div>
