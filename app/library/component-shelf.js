...
export const Shelf = component(({ leftList, root }) => {
  ...
  if (root) {
    ...
    if (
      root.style.getPropertyValue('--right-library-sidebar-width') === '0px'
    ) {
      expanded = false
    }
  }
  const context = { menuEl, buttonEl, root }
  return html`<button ref="${buttonEl}" class="Button" aria-expanded="${expanded}" onclick=${event =>
    toggleMenu(event, context)}>Uploads</button>
  <div class="Library-shelf App-sidebar App-sidebar--right" hidden="${!expanded}" ref=${menuEl}>
  <div class="App-menu"><ol class="App-menu-list"><li><button class="App-sidebar-closer App-button" aria-label="Close library shelf sidebar" onclick=${event =>
    toggleMenu(
      event,
      context
    )}>&times;</button></li><li><h2 class="App-title">Uploads</h2></li><li><details class="MenuButton">
...
<input type="file" hidden onchange="${() =>
    ...
    })}" name="file-selector" id="file-selector" accept=".epub,application/epub+zip" multiple></label>
  </details-menu>
  </details></li></ol></div>
  <file-drop accept="application/epub+zip" class="Library-file-drop" id="shelf-file-drop" onfiledrop="${event =>
    dispatch({ type: 'files', dispatch, files: event.files })}" multiple>
  <div data-shelf-uploads=${
  state && state.length !== 0 ? 'loading' : 'not-loading'
}><h3 class="App-nav-label">In Progress
  <span class="Spinner">
