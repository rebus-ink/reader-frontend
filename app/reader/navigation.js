...
export const NavButton = component(({ book, request }) => {
  ...
  if (root) {
    ...
    if (root.style.getPropertyValue('--reader-contents-width') === '0px') {
      expanded = false
    }
  }
  ...
  return html`<li>
  <button ref="${buttonEl}" aria-expanded="${expanded}" class="App-button" aria-label="Show left sidebar" onclick=${event =>
  toggleMenu(
    event,
    context
  )}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="square" stroke-linejoin="round"><line x1="3" y1="12" x2="21" y2="12"></line><line x1="3" y1="6" x2="21" y2="6"></line><line x1="3" y1="18" x2="21" y2="18"></line></svg></button>
...
export async function getNavigation (book, oldNav, setState, act = activities) {
  ...
  if (html) {
    ...
    const data = {
      ...
      content: `<ol class="Contents-list">${fallback
        .map((chapter, index) => {
          if (!chapter.name) {
            chapter.name = `Chapter ${index + 1}`
          }
          chapter.href = makeChapterURL(path, chapter, false)
          return `<li class="Contents-item"><a  class="Contents-link" href="${
            chapter.href
          }">${chapter.name}</a></li>`
        })
        .join('')}</ol>`
